...
namespace Forms_TP_Labo_2_Sysacad.Forms.Admin
{
    public partial class FormEditarPromedioRequerido : FormPadre
    {
        ...
        private async void editarBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                { "promedio", _promedio.ToString() },
                { "creditos", _cursoSeleccionado.CreditosRequeridos.ToString() }
            };

            RespuestaValidacionInput respuestaValidacion = await gestorRequisitos.GestionarConfirmacionRequisitos(dictCampos, _cursoSeleccionado.CodigoFamilia, _cursoSeleccionado.Correlatividades, _cursoSeleccionado.CreditosRequeridos.ToString(), _promedio);
