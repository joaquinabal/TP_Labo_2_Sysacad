...
namespace Forms_TP_Labo_2_Sysacad.Forms.Admin
{
    public partial class FormEditarCreditosRequeridos : FormPadre
    {
        ...
        private async void editarBtn_Click(object sender, EventArgs e)
        {
            ...
            RespuestaValidacionInput respuestaValidacion = await gestorRequisitos.GestionarConfirmacionRequisitos(dictCampos, _cursoSeleccionado.CodigoFamilia, _cursoSeleccionado.Correlatividades,_creditos, _cursoSeleccionado.PromedioRequerido.ToString());
            if (respuestaValidacion.ExistenciaErrores)
            {
                MessageBox.Show(respuestaValidacion.MensajeErrores);
            }
            else if (!respuestaValidacion.AusenciaCamposVacios)
            {
