...
{
    public partial class FormEditarCreditosRequeridos : FormPadre
    {
        ...
        public FormEditarCreditosRequeridos(Curso curso)
        {
            InitializeComponent();
            ...
            credRequeridoTextBox.Text = curso.CreditosRequeridos.ToString();
        ...
        private void credRequeridoTextBox_TextChanged(object sender, EventArgs e)
        {
            _creditos = credRequeridoTextBox.Text;
        }

        private async void editarBtn_Click(object sender, EventArgs e)
        {
            ValidadorInputGenerico validarPromedio = new ValidadorInputGenerico();

            GestorRequisitos gestorRequisitos = new GestorRequisitos();

            Dictionary<string, string> dictCampos = new Dictionary<string, string>()
            {
                { "promedio",  _cursoSeleccionado.PromedioRequerido.ToString() },
                { "creditos", _creditos }
            };


            RespuestaValidacionInput respuestaValidacion = await gestorRequisitos.GestionarConfirmacionRequisitos(dictCampos, _cursoSeleccionado.CodigoFamilia, _cursoSeleccionado.Correlatividades, int.Parse(_creditos), _cursoSeleccionado.PromedioRequerido);
            ...
            {
                MessageBox.Show("Créditos editado correctamente.");
                this.Close();
